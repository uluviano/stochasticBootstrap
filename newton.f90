...
program MC
   ...
   implicit none
   integer NT,nZFile,nZ,nSpin, nPoints,seedInput,iinfo,invlwork, lda
   integer, allocatable :: nZList(:)
   integer nops
   integer, allocatable :: spinh(:)
   real*8, allocatable :: op_bounds(:,:)
   real*8, allocatable :: DDini(:),DDn(:),opstep(:)
   real*8, allocatable :: invWork(:),zz(:,:),zzt(:,:)
   real*8, allocatable :: gg1(:,:,:,:),gg2(:,:,:,:)
   real*8  Dstar,Dext,Temp
   real*8, allocatable :: lambda(:),eps(:),z1(:),z2(:),err(:),grad_t(:),grad(:),tempMat(:,:),invP(:,:)
   real*8, allocatable :: eigenvecs(:,:),hessiant(:,:),hessian(:,:),rho(:),rhon(:),DD(:),DDi(:,:),step(:)
   real*8, allocatable :: varMatrix(:,:)
   real*8, allocatable :: gt1(:),gt2(:),b(:),c(:),d(:),gradIndices(:)
   real*8, allocatable :: MM(:,:),AA(:,:),res(:)
   integer, allocatable :: seed(:)
   character*200   filepar
   real*8, allocatable :: eigen(:),wk(:),deltaDD(:)
   integer ier



   integer i,j,jj,k,ii,it,acc,positiveOPEs,areOPEspos,invSampleDensity,nGoodZ,validPoint,externalInOPE,nVars,newtonTag
   real*8 DD_tt,DD_t,g1,g2,DDmin,DDmax,SS,dx,actt,actn,finiteStep,r,coleps,wall,adaptStep,lambda0,zPointDensity,gammaN,eigEpsilon
   !functions
   real*8 relu, RHOf
   adaptStep=1.00001d0
   ...
   allocate(DD(0:nops),DDn(0:nops),varMatrix(0:nops,0:nops)) ! this includes the external operator (DD(1))
   allocate(hessian(nVars,nVars),grad_t(nVars),grad(nVars)) ! this includes the external operator (DD(1))
   allocate(hessiant(nVars,nVars), invP(nVars,nVars),tempMat(nVars,nVars), eigenvecs(nVars,nVars))
   allocate(AA(nops,nops),rho(nops),rhon(nops))
   allocate(res(nZ),MM(nZ,nops))
   allocate(eigen(nVars),wk((nVars)**3),deltaDD(nVars))
   ...
   DDn(:)=DDini(:)
   do it=1,NT
      call compute_grad(grad,DD,finiteStep,actt,rho)
      if(MOD(it,invSampleDensity).eq.0)write(6,'(t1,2000f29.16)')DD(:),actt,rho(:),grad(:)


      ii=1
      do i=0,nops
         if(i.eq.gradIndices(ii))then
            DD_tt = DD(i)
            DD(i) = DD(i) + finiteStep
            call compute_grad(grad_t,DD,finiteStep,actt,rho)
            hessian(:,ii)=grad_t(:)
            DD(i) = DD(i) - 2*finiteStep
            call compute_grad(grad_t,DD,finiteStep,actt,rho)
            hessian(:,ii)=( hessian(:,ii) - grad_t(:))/(2*finiteStep)
            DD(i) = DD_tt
            ii = ii +1
         endif
      enddo
      !Symmetrize hessian to reduce numerical noise
      do i =1,nVars-1
         do j =i+1,nVars
...
!     enddo
!     stop
      !call invert(hessiant, nVars + 1 )
      ! add damping
      do i=1,nVars
         hessian(i,i) = hessian(i,i)+ 1/gammaN 
      enddo
      invlwork = nVars * 64
      allocate(invWork(invlwork))
      tempMat(:,:)=hessian(:,:)
      lda=nVars
      Call dgetrf(nVars, nVars, hessian, lda, eigen, iinfo)
      !write(6,*)iinfo
 call dgetri(nVars, hessian, lda, eigen, invWork, invlwork, iinfo)
 deallocate(invWork)

 !check
 !write(6,*) matmul(tempMat(:,:),hessian(:,:))
      ...
      if(newtonTag.eq.1)then
         deltaDD(:)=matmul(hessian(:,:),grad(:))
      else
         deltaDD(:)=grad(:)
      endif
      ii=1
      do i=0,nops
         if(i.eq.gradIndices(ii))then
            DDn(i) = DD(i) - deltaDD(ii)
            !DDn(i) = DD(i) -gammaN * grad(ii)
            !DDn(i) = DD(i) +0.000000001 * grad(ii)
            ii = ii+1
         else
            DDn(i) = DD(i)
         endif
      ...
      !write(6,'(t1,2000f29.16)')DDn(:),actn,gammaN
      if(gammaN<0.00000000001d0)then
         write(6,*)1
         write(6,'(t1,2000e29.16)')DD(:),actt,rho(:),grad(:)
         stop
      end if
...
deallocate(MMren,jpvt,work)
   ...
   subroutine compute_grad(grad,DD,step,ss_i,rho_i)
   implicit none
   integer i,ii
   real*8 DD(0:nops),grad(nVars), ss_i, rho_i(nops), rho_t(nops), SS_t, step
   call compute_SS(SS_i,DD,rho_i)
   !!!
   ...
   do i=0,nops
      if(i.eq.gradIndices(ii))then
         DD_t = DD(i)
         DD(i) = DD(i) + step
         call compute_SS(SS_t,DD,rho_t)
         grad(ii) = ss_t 
         DD(i) = DD(i) - 2*step
         call compute_SS(SS_t,DD,rho_t)
         grad(ii) = ( grad(ii)- ss_t )/(2*step)
         DD(i) = DD_t
         ii = ii+1
      endif
   enddo
