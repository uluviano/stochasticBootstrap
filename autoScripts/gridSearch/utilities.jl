...
function extendDDsforPlot(sec,dd)
        extendedDDs = [dd[1]]
        i=1
        for ll in sec
                push!(extendedDDs,dd[i+1:i+ll]...)
                push!(extendedDDs,dd[i+1])
                i+=ll
...
function chainGoesDown(chain) 
    for i in 2:length(chain)
        n1,n2 = sum(chain[i-1][1] )+ 2 ,sum(chain[i][1] )+ 2
        chain[i-1][2][n1] <  chain[i][2][n2] && return false
    end
    return true
end
...
function nonRedundantMinima(sec::Vector{Int},minima::Vector{Any},relTol::Float64)  
    ...
    for i in 1:length(minima)
        closeCalls = findall(x->x<relTol, distMatrix[i,i+1:end]) .+ i
        for j in closeCalls
...
function plotSpectrum(refs::Vector{Matrix{Float64}}, minimaToPlot,nameTag::String)
    ...
    #make things pretty
    spinOffsetStep = 1.7/(length(minimaToPlot)+2)
    ...
    function scatterRef(refSecVec::Vector{Int64}, dataMatrix)
        ...
        refTemp = vcat(dataMatrix[1:1,:],vcat([ dataMatrix[(dataMatrix[:,2].==spin),:][1:mult,:] for (spin, mult) in spinPair]...))

        traces = scatter(
            x=vcat([-2],refTemp[2:end,2]).-0.2,
            y=refTemp[1:end,1],
            mode="markers",
            name="Ising",
            marker=attr(
                ...
                #text= string.(Int.(refIsingTemp[3:end,2])) , 
    ...
    referencePlots = [
    scatterRef(refSecVec, dataMatrix)
    for dataMatrix in refs
    ]
    ...
    pp = plot(
        vcat(referencePlots
            ,
    [
    scatter(y=extendDDsforPlot(sec,dd), x=vcat([-2], spinVec(sec .+ 1)) .+ spinOffsetStep*(ii), mode="lines+markers",
    marker=attr(
                size=15,
                color=colorsMinima[ii],
                symbol=:diamond
    ),
        name = "$ii",
                showlegend=true
    ...
    ...),
    Layout(
        #title = "minimalModel-like minima, λ0 = 0.42 Δσ = $(round(dsig,digits=4)), ΔT fixed, relTol=$relTol, maxMismatch=$maxMismatch",
        title = nameTag,


        xaxis_title="Spin", yaxis_title="Δ", xaxis=attr(showticklabels=false,zeroline=false),
    ...
    annotations=
        vcat([
            ...
            for spin in 0:2:(2*length(refSecVec) -2)
        ])
    )
    )
end
...
#Quality assesments
function getQuality(reference::Vector{Float64}, (sec,dd) )
    test = [ dd[2],  dd[sec[1]+2], dd[sum(sec)+3], dd[sum(sec)+sec[1]+3] ]
#@show test
    return sqrt(sum( ((reference-test) ./ reference).^2)) / length(test)
