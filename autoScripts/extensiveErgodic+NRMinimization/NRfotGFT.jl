...
inputFileList = []
        dirString = "2022b22-dsig$dsig-dtFixed$DTfixed"
        mkpath(dirString)
        cd(dirString)
        for ff in files
            cp("../$ff", ff)
...
sector = [count(x->x==l,spinVector) for l in sort(unique(spinVector))]
        spinPair = [(2.0*(i-1), sector[i]) for i in 1:length(sector)]
spec = [ 2dsig + spin + 2*(i-1) for (spin, mult) in spinPair for i in 1:mult]
		...
		#   generate input 
						...
						bLo =  (stDim-2) / 2 
						bHi = stDim
				operatorsInput = ["-1.	    $bLo	$bHi	$dsig	0. \n"]
				operatorsInputlowT = ["-1.	$bLo	$bHi	$dsig	0. \n"]
			...
			for (opI,l) in enumerate(spinVector)
				...
				push!(movingOps,step)
			push!(operatorsInputlowT,"$l	$bLo	$bHi	$(spec[opI])	$step \n")
